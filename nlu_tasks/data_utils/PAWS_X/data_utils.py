    ...
    if os.path.exists(data_path):
        ...
        for cur_type in ['train', 'dev', 'test']:
            ...
            with open(os.path.join(data_dir, f'{cur_type}.tsv'), "r", encoding="utf8") as fr:
                ...
                for i, line in tqdm(enumerate(raw_dataset), desc=f"* Loading {cur_type.upper()} set...",
                                    bar_format="{l_bar}{bar:10}{r_bar}", total=len(raw_dataset)):
                    if i == 0:
                        continue
        ...
        for d_type in total_dataset:
            if d_type == 'label_map':
                continue
            assert len(total_dataset[d_type]['sentence1']) == len(total_dataset[d_type]['sentence2']) == len(
                total_dataset[d_type]['label'])
