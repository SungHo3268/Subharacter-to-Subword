...
class SUB2_LoRA_Layer(nn.Module):
    ...
    def __init__(self, tokenizer, sub2_tokenizer, original_layer, config: SUB2_Config):
        ...
        self.sub2_norm_layer = nn.LayerNorm(config.hidden_dim)
    ...
    def forward(self, x):
        ...
        # print(f"original embedding average: {original_embedding.mean()}")
        # print(f"sub2 embedding average: {sub2_embedding.mean()}")
        ...
        if self.config.is_bert:
            ...
            sub2_embedding = self.sub2_norm_layer(sub2_embedding)
