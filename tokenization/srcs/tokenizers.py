...
class SubwordTokenizer(BaseTokenizer):
    ...
    def train_model(self, args):
        ...
        train_cmd += f"--pad_id=-1 --bos_id=-1 --eos_id=0 --unk_id=1 "
...
class StrokeTokenizer(BaseTokenizer):
    def __init__(self, config_path):
        ...
        self.bos_token = self.config["bos_piece"]
        self.eos_token = self.config["eos_piece"]
        self.unk_token = self.config["unk_piece"]
        self.pad_token = self.config["pad_piece"]
        self.special_tokens = [
            self.bos_token,
            self.eos_token,
            self.unk_token,
...
class BtsTokenizer(BaseTokenizer):
    def __init__(self, config_path):
        ...
        self.eos_token = self.config["eos_piece"]
        self.unk_token = self.config["unk_piece"]
        self.pad_token = self.config["eos_piece"]
        self.special_tokens = [
            self.bos_token,
            self.eos_token,
            self.unk_token,
...
class StrokeVarTokenizer(BaseTokenizer):
    ...
    def __init__(self, config_path):
        ...
        self.cons_len = 4
        self.vow_len = 1
        self.char_len = self.cons_len + self.vow_len + self.cons_len    # chosung + joongsung + jongsung
...
class JamoVarTokenizer(BaseTokenizer):
    def __init__(self, config_path):
        super().__init__(config_path)
...
if __name__ == '__main__':
    ...
    # _tokenizer = JamoVarTokenizer(config_path="tokenization/utils/tok_info.cfg")
